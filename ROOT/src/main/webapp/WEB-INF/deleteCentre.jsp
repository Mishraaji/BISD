...
<div class="large-12 columns" >
	<s:form action="/deleteCenter1.action" method="post" style="width:100%" name="addCenter" id="addCenter" validate="true">
		<div class="customHeader">
	<i class="step fi-home size-24"></i>  &nbsp;&nbsp; Delete Centre
	</div>
	 <div style="height:5%;">
    
    </div>
		<s:textfield name="name" id="name" label="Centre Name" required="true" 
			value="%{#session.CURRENT_CENTRE.name}"  readonly="true"></s:textfield>
		<s:textfield name="centerCode" id="centerCode" label="Centre Code" required="true" 
			value="%{#session.CURRENT_CENTRE.centerCode}"  readonly="true"></s:textfield>
		<s:textarea name="address" id="address" label="Address" rows="4" cols="20" required="true"
			value="%{#session.CURRENT_CENTRE.address}"  readonly="true"></s:textarea>
		<s:textfield name="city" id="city" label="City" required="true"
			value="%{#session.CURRENT_CENTRE.city}"  readonly="true" ></s:textfield>
		<s:select label="State" name="state" id="state" value="%{#session.CURRENT_CENTRE.state}"
																	 list="{ 'Andaman & Nicobar' ,
																			...
																			'Dadra and Nagar' , 
																			'Daman and Diu' ,	
																			'Lakshadweep' 	}"  readonly="true">
					</s:select>
		 <s:textfield name="pinCode" id="pinCode" label="Pin Code" size="20" 
		 	required="true" maxlength="6" value="%{#session.CURRENT_CENTRE.pinCode}"  readonly="true"/>
		 <s:textfield name="adminName" id="adminName" label="Admin Name"   size="20" required="true"
		 	value="%{#session.CURRENT_CENTRE.adminName}"  readonly="true"/>
		 <s:textfield name="email" id="email" label="Admin Email"   size="20" required="true"  
		 	placeholder="This email will serve as user name for the admin" value="%{#session.CURRENT_CENTRE.email}" readonly="true"/>
