...
<div class="large-12 columns" >
	<s:form action="editUser1.action" method="post" style="width:100%" name="addCenter" id="addCenter" validate="true">
		<div class="customHeader">
	<i class="step fi-home size-24"></i>  &nbsp;&nbsp; Edit User
	</div>
	 <div style="height:5%;">
    
    </div>
    	<s:select list="#session.CENTRE_LIST" label=" Select Centre " id="ccCode" name="ccCode"
    		value="%{#session.CURRENT_USER.ccCode}"></s:select>
		<s:textfield name="userName" id="userName" label="User Name" required="true" 
			value="%{#session.CURRENT_USER.userName}" ></s:textfield>
		...
		<s:textfield name="email" id="email" label="User Email"   size="20" required="true"  
		 	...
		 	value="%{#session.CURRENT_USER.email}" />
		<s:select list="#session.ROLES" label=" Select Role " id="roleId" name="roleId"
			value="%{#session.CURRENT_USER.roleId}"></s:select>
		...
		<br/>
		<s:hidden name="operation" value="editUSer"></s:hidden>
